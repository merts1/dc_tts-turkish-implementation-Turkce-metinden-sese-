...
from data_yukle import batch_al, harf_yukle
...
class Graph:
    def __init__(self, num=1, mode="train"):
        ...
        '''
        # Load vocabulary
        self.char2idx, self.idx2char = harf_yukle()
        ...
        if mode=="train":
            self.L, self.mels, self.mags, self.fnames, self.num_batch = batch_al()
            self.prev_max_attentions = tf.ones(shape=(hp.B,), dtype=tf.int32)
            self.gts = tf.convert_to_tensor(guided_attention())
        else:  # Synthesize
            self.L = tf.placeholder(tf.int32, shape=(None, None))
            self.mels = tf.placeholder(tf.float32, shape=(None, None, hp.mels_s))
        ...
        if training:
            ...
            # Training Scheme
            self.lr = learning_rate_decay(hp.LEARNING_RATE, self.global_step)
